    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Task.Factory.StartNew(() => action("alpha")).
                ...
                ContinueWith(antecendent => action("gama"));
            ...
            Task.Factory.StartNew<int>(() => negate(5)).ContinueWith(p => negate(p.Result)).ContinueWith(p => negate(p.Result));

            Console.WriteLine("Creating a Mix task");
            Task.Factory.StartNew<int>(() => negate(88)).
                ContinueWith(antecendent => action("test")).
                ContinueWith(antecendent => negate(66));
